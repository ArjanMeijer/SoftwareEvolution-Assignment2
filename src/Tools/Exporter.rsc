...
int MIN_CLONE_LENGTH = 1;

map[int, list[tuple[list[STEdge], str]]] CloneClasses = (); 	
...
public list[str] CloneToText(list[tuple[list[STEdge], str]] clones, map[int, str] rIndex, list[int] values) {
	list[str] result = [];
...
public void SaveClass(int saveToID, list[STEdge] edges) {
	...
	str fileName = GetFile(leafNode).path;

	if(saveToID notin CloneClasses)
		CloneClasses += (saveToID: [<edges, fileName>]);
	else
		CloneClasses[saveToID] += [<edges, fileName>];
}
